...
const (
	// CRIVersion is the CRI version supported by this version of the CRI plugin.
	CRIVersion = "v1alpha2"
)
...
func initCRIService(ic *plugin.InitContext) (interface{}, error) {
	...
	log.G(ctx).Infof("Start cri v1alpha2 plugin with config %+v", c)
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to create CRI v1alpha2 service")
	}

	go func() {
		if err := s.Run(); err != nil {
			log.G(ctx).WithError(err).Fatal("Failed to run CRI v1alpha2 service")
		}
	}()
	return s, nil
}
